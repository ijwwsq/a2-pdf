# a2pdf — фирменные документы из markdown, Word и Notion

Сервис и CLI, которые превращают `.md`, `.docx`, вставленный markdown или
ссылку на страницу Notion в документ в оформлении A2DATA: обложка, колонтитулы
с нумерацией страниц, диаграммы mermaid в фирменной палитре, встроенные
шрифты Inter и JetBrains Mono.

На выходе — **PDF** или **Word**.

PDF — витрина: обложка с фирменным фоном или фотографией, диаграммы,
встроенные шрифты. Word — деловой корпоративный шаблон: сдержанная титульная
страница, колонтитулы с логотипом и нумерацией, редактируемый текст. Картинкой
в Word вставляются только диаграммы, которые Word рисовать не умеет.

Логотип во всех форматах — графика из брендбука (буквы переведены в кривые),
поэтому он одинаков везде и не зависит от установленных шрифтов. Текст в Word
набирается системными шрифтами: фирменных Inter, Oswald и Roboto Condensed
нет на каждой машине, а подстановка засечек испортила бы документ.

## Организации

Сервис верстает для двух брендов, оформление выбирается на лету:

| Ключ | Организация | Основной цвет | Акцент | Шрифты |
|---|---|---|---|---|
| `a2data` | A2DATA | `#0B2660` | `#1FA8FC` | Inter |
| `becloud` | BeCloud.AI | `#2F3586` | `#4A6BFF` | Oswald + Roboto Condensed |

В форме организация переключается рядом с выбором источника, в API — полем
`brand`, в CLI — ключом `--brand becloud`. Новый бренд добавляется правкой словаря
в `a2pdf/brands.py`: цвета, шрифты, логотип и реквизиты.

## Доступ

Сервис закрыт логином и паролем. Пароль хранится только хешем: сгенерируйте
его и положите значения в окружение.

```bash
python -m a2pdf.auth
```

| Переменная | Что это |
|---|---|
| `A2PDF_AUTH` | `off` — вход отключён (закрытый контур, доступ ограничен сетью) |
| `A2PDF_USER` | имя пользователя, по умолчанию `admin` |
| `A2PDF_PASSWORD_HASH` | scrypt-хеш пароля |
| `A2PDF_SECRET` | секрет для подписи сессий |
| `A2PDF_SESSION_HOURS` | срок жизни сессии, по умолчанию 12 часов |

Как это работает: пароль проверяется scrypt-хешем, сессия хранится в куке
`HttpOnly` c подписью HMAC-SHA256 и сроком жизни, по `https` кука уходит
с флагом `Secure`. Пять неудачных попыток с адреса за пять минут — и вход
блокируется на это окно. Формы проверяются на совпадение `Origin` с хостом.

Пока `A2PDF_PASSWORD_HASH` не задан, сервис отвечает только с локальной машины
и пишет предупреждение в лог — открытым в сеть он не окажется.

```
a2pdf/
  a2pdf/
    brands.py        описания организаций: цвета, шрифты, логотипы
    auth.py          вход: хеш пароля и подписанные сессии
    core.py          вёрстка и сборка PDF
    docx_reader.py   чтение .docx
    docx_writer.py   сборка фирменного .docx
    assets/logo/     логотип в кривых (svg и png)
    html_reader.py   HTML в блоки документа
    notion.py        импорт страниц Notion
    fetch.py         загрузка обычных страниц и raw markdown
    web.py           HTTP-сервис (FastAPI)
    static/          веб-форма
    assets/          шрифты и mermaid
  Dockerfile
  docker-compose.yml
  md-to-pdf.cmd      перетащи файл на этот значок (Windows)
  requirements.txt
```

## Поднять сервис

```bash
docker compose up -d --build
```

Открыть http://localhost:8000 — форма загрузки. Проверка живости:
`GET /healthz`, описание API: `/api`.

Образ самодостаточный: внутри chromium, шрифты и mermaid, в интернет
на старте сервис не ходит.

### Переменные окружения

| Переменная | По умолчанию | Что делает |
|---|---|---|
| `A2PDF_WORKERS` | 2 | сколько документов собирается одновременно |
| `A2PDF_TIMEOUT` | 120 | предел на один документ, секунд |
| `A2PDF_MAX_UPLOAD` | 20971520 | максимальный размер загрузки, байт |
| `A2PDF_CHROME` | автопоиск | путь к chromium или chrome |
| `A2PDF_ASSETS` | `a2pdf/assets` | где лежат шрифты и mermaid |
| `A2PDF_TMP`, `A2PDF_OUT` | системный temp | рабочие каталоги |
| `NOTION_TOKEN` | — | токен интеграции Notion для закрытых страниц |
| `A2PDF_USER`, `A2PDF_PASSWORD_HASH`, `A2PDF_SECRET` | — | вход в сервис, см. «Доступ» |

Один воркер держит один процесс chromium, поэтому память растёт линейно:
на 2 воркера достаточно 1 ГБ. Ставьте `A2PDF_WORKERS` по числу ядер, а перед
сервисом — обычный reverse proxy с ограничением размера тела запроса.

## Как пользоваться

**Через браузер.** Три вкладки: перетащить файл, вставить markdown текстом
или дать ссылку. При необходимости раскрыть «Оформление документа» и нажать
«Собрать PDF».

Во вкладке «Текст» переключатель выбирает, что вставлено: markdown или схема
mermaid. Схему сервис сам обернёт в блок кода, предложит скачать картинкой
и покажет свои шаблоны оформления вместо обложек документа.

**Через API.**

```bash
curl -X POST http://localhost:8000/convert -F "file=@doc.md" -F "kicker=Коммерческое предложение" -F "index=01" -F "meta=Клиент=ООО Пример;Срок=10 недель" -o doc.pdf
```

Источник задаётся одним из полей: `file`, `text` или `url`.

```bash
curl -X POST http://localhost:8000/convert -F "url=https://www.notion.so/..." -o doc.pdf
```

Формат результата задаёт поле `format`: `pdf` (по умолчанию), `docx`
или `png` — картинка первой схемы документа.

```bash
curl -X POST http://localhost:8000/convert -F "text=$(cat shema.mmd)" -F "format=png" -F "scheme=dark" -o shema.png
```

Остальные поля необязательные и перекрывают front matter: `title`, `subtitle`,
`kicker`, `index`, `header`, `footer`, `confidential`, `meta`
(строки `Ключ=Значение` через перевод строки или `;`), `style=light|dark`,
`background` (имя встроенного фото), `photo` (файл), `cover=0`, `numbered=0`,
`brand` (`a2data` или `becloud`), `font` (набор шрифтов), `scheme` (шаблон схемы).

### Ссылки

Notion читается через его API — подходят адреса `notion.so`, `notion.site`
и `app.notion.com`. Страница должна быть открыта по ссылке: Share → General
access → **Anyone on the web with link**. Для закрытых страниц заведите internal
integration, передайте сервису `NOTION_TOKEN` и добавьте интеграцию к странице
через Share.

Обычные страницы и ссылки на `.md` берутся простым HTTP-запросом. Браузер при
этом не запускается вообще, поэтому страницы, где текст подгружают скрипты,
прочитать нельзя — такие проще сохранить в файл и загрузить им.

**Локально, без сервиса.**

```bash
pip install -r requirements.txt
```

```bash
python -m a2pdf документ.md -o готовый.pdf --kicker "Отчёт" --confidential
```

Тот же документ в Word — с ключом `--docx`:

```bash
python -m a2pdf документ.md --docx
```

На Windows можно просто перетащить файлы на `md-to-pdf.cmd`.

**Как библиотека.**

```python
import a2pdf, pathlib

a2pdf.build_any(pathlib.Path("документ.docx"), pathlib.Path("out.pdf"),
                overrides={"kicker": "Отчёт", "meta": {"Автор": "Егор"}})
```

## Что понимает конвертер

Из markdown: заголовки, абзацы, списки, таблицы, код, цитаты (рисуются
янтарной выноской), картинки, блоки ` ```mermaid `.

Из docx: заголовки, жирный и курсив, списки, таблицы, цитаты, картинки.
Оформление Word игнорируется — документ пересобирается по брендбуку.

Из Notion: заголовки, абзацы, списки, чек-листы, цитаты и колауты, код,
таблицы, разделители, картинки и вложенные блоки.

Из веб-страницы: то же, что из HTML, — заголовки, текст, списки и таблицы.

### Шрифты

По умолчанию документ набирается шрифтами брендбука. Набор можно сменить:
в форме — полем «Шрифт документа», в API — полем `font`, в CLI — `--font`.

| Ключ | Набор |
|---|---|
| `inter` | Inter |
| `oswald` | Oswald + Roboto Condensed |
| `manrope` | Manrope |
| `roboto` | Roboto |
| `serif` | PT Serif + PT Sans |

### Диаграммы

Блоки ` ```mermaid ` рисуются в фирменной палитре. Высокая схема сжимается
так, чтобы целиком поместиться на страницу — на части она не разрывается.
Разбитая схема не роняет сборку: вместо неё в документ попадает исходник,
чтобы автор увидел, что чинить.

Шаблон схемы задаёт поле `scheme` и меняет не только подложку, но и цвета
элементов — заливку узлов, обводку, линии и подписи. Свои цвета в `classDef`
подменяются палитрой шаблона, так что группы узлов остаются различимыми.

| Шаблон | Как выглядит |
| --- | --- |
| `outline` | белый фон, узлы без заливки, тонкая обводка |
| `soft` | светлая подложка, пастельные плашки, мягкая тень |
| `solid` | узлы залиты фирменным цветом, текст белый |
| `dark` | тёмный фон, светлые линии и подписи |
| `clear` | прозрачный фон — PNG с альфа-каналом для слайдов |

### Обложка

По умолчанию синяя. `style: light` даёт светлый вариант, `photo` кладёт
снимок фоном: он обесцвечивается и уходит под синий слой — получается дуотон
в фирменном цвете.

Front matter в начале md задаёт обложку:

```markdown
---
title: Rate Limiter
subtitle: Ограничение частоты запросов
kicker: Тестовое задание
index: "01"
confidential: Не для кандидатов
header: Тестовое задание · Rate Limiter
footer: Python Backend Developer · a2data.ai
cover: true
numbered: true
mermaid: true
style: dark
photo: cover.jpg
meta:
  Роль: Python Backend
  Таймбокс: 4 часа
---
```

Маркеры внутри текста:

- `<!--PART:Часть 2|Заголовок-->` — разделитель между частями документа;
- `<!--CAP:подпись под схемой-->` — подпись к диаграмме;
- `<!--NUMBERING:off-->` — выключить нумерацию разделов с этого места.

## Логотип

Логотип собирается один раз из фирменного Inter в кривые:

```bash
python tools/make_logo.py
```

Результат лежит в `a2pdf/assets/logo/` — цветной и белый варианты в SVG и PNG.

## Тесты

```bash
pytest            # быстрые: разбор, пресеты, вход, HTTP-слой
pytest -m slow    # сборка целиком, запускает Chrome
```

## Как это устроено

Markdown, docx, Notion или веб-страница разбираются в единый список блоков;
блоки превращаются в HTML со стилями брендбука, headless chromium печатает его
в PDF, PyMuPDF склеивает обложку с телом и штампует колонтитулы с номерами
страниц. Браузер используется только для печати.

Палитра и шрифты берутся из брендбука: navy `#0B2660`, blue `#1FA8FC`,
amber `#FF9F1C`, Inter + JetBrains Mono.

Модули: `core` — разбор markdown и вёрстка PDF, `diagrams` — оформление схем,
`docx_writer` — сборка Word, `brands` — палитры и шрифты организаций,
`notion`, `fetch`, `html_reader`, `docx_reader` — чтение источников,
`web` — HTTP-сервис, `auth` — вход.
